<div id="models-modal" tabindex="-1" wire:ignore.self
    ...
    <div class="relative p-4 w-full max-w-[50rem] max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 flex-row-reverse gap-2 flex md:p-5">
                ...
                <form class="space-y-4 flex-grow" wire:submit.prevent="createModel" wire:loading.remove>
                    <div>
                        <input wire:model.defer="input.model_name" type="text" name="dealer" id="dealer"
                            class="
                            @error('model_name')
                    ...
                    <div>
                        <select id="countries"
                            wire:model.defer="input.manufacturer"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option selected>Choose a Manufacturer</option>
                            @foreach ($manufacturers as $manufacturer)
                            <option value="{{ $manufacturer->id }}">{{ $manufacturer->partner_name }}</option>
                            @endforeach
                        </select>
                        @error('manufacturer')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
