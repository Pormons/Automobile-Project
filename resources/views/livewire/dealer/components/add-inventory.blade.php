<div id="vehicle-dealer-modal" tabindex="-1" wire:ignore.self
    ...
    <div class="relative p-4 w-full max-w-[80rem] max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-2 md:px-3 border-b rounded-t dark:border-gray-600">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                    Add Vehicle to your Inventory {{ $id }}
                </h3>
                <button type="button"
                ...
                <div id="dropdownSearch" class="z-10 bg-white rounded-lg shadow w-60 dark:bg-gray-700">
                    ...
                    <div class="px-3 py-1">
                        <div class="relative w-full">
                            <select id="brands" wire:model.live="searchBrand"
                                class="bg-transparent mb-1 w-full border-2 rounded-lg focus:ring-0 focus:outline-none outline-none">
                                <option value="">Brands</option>
                                @foreach ($brands as $brand)
                                    <option value="{{ $brand->id }}">{{ $brand->brand_name }}</option>
                                @endforeach
                            </select>
                            <select id="models" wire:model.live="searchModel"
                    ...
                    <ul class="px-3 h-80 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                        @foreach ($vins as $vin)
                            <li class="hover:cursor-pointer cursor-pointer">
                                <div wire:click="selectVin('{{ $vin->vin }}')"
                                    class="hover:cursor-pointer flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                                    <input wire:model.defer="selectedVins" value="{{ $vin->vin }}" type="checkbox"
                                        value=""
                                        class="hover:cursor-pointer cursor-pointer w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                    <label for="checkbox-item-11"
                                        class=" hover:cursor-pointer cursor-pointer w-full ms-2 text-sm font-medium text-gray-900 rounded dark:text-gray-300">
                                        {{ $vin->vin }}
                                    </label>
                                </div>
                            </li>
                    ...
                    <a href="#" wire:click="addVehicles({{ $id }})"
                ...
                <div class="flex flex-grow flex-row p-4" wire:loading.remove.delay.longest>
                    @if ($selectedVin)
                        ...
                        <div class="p-4 flex flex-col flex-grow items-center">
                            <Span
                                class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $selectedVehicle->model_info->brand_info->brand_name }}</Span>
                            <Span
                                class="mb-2 text-base font-bold tracking-tight text-gray-900 dark:text-white">{{ $selectedVehicle->model_info->model_name }}</Span>
                            ...
                            <div id="specifications" name="specifications"
                                class="grid justify-items-start gap-6 grid-cols-2">
